...
def create_test_dataset():
    ...
    df = pd.DataFrame({
        ...
        "PhD Defense Date": [(START_DATE + timedelta(days=i)).strftime('%Y-%m-%d') for i in range(NUM_ROWS)],
        "Employment Start Date": [START_DATE.strftime('%Y-%m-%d')] * NUM_ROWS,
        "Employment Termination Date": [(START_DATE + timedelta(days=NUM_ROWS)).strftime('%Y-%m-%d')] * NUM_ROWS,
        "Birth Date": [(START_DATE - timedelta(days=365 * 25)).strftime('%Y-%m-%d')] * NUM_ROWS,
